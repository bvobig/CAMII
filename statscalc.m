...
% original "if startdata<enddata" skipping the loop entirely.)
    segrange = startdata:enddata-1;
    [inrange_tf, inrange_grp] = ismember(obsType(segrange), rawtypenames);
    segtonext = obsToNext(segrange);
    inrange_sums = accumarray(inrange_grp(inrange_tf), segtonext(inrange_tf), [19, 1]);
...
catsums = accumarray(cat_grp(cat_tf), obsToNext(cat_tf), [19, 1]) / interactiontimetotal;
