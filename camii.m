...
[data, segmentsbv, feats, stats, types, typestotal] = analysis (improdata, camii_model); %Analyse Data
exportResults(data, clno, segmentsbv, feats, stats, types, typestotal, graphIDs, segIDs, statIDs, typeIDs, options) %Export Data
...
function [data, segmentsbv, feats, stats, types, typestotal] = analysis (mttbdata, model) %typestruct
    ...
    stats = statscalc (obsc, obst, predsc, predst, data, 1, 100); disp("Stat Calculation finished") % calculate stats %buffer*2 is frame in .1s (default = 25)
...
function exportResults(data, clno, segmentsbv, feats, stats, types, typestotal, graphIDs, segIDs, statIDs, typeIDs, options)

    if options.GraphExport
        graphexport(data, segmentsbv, graphIDs, clno, "-jpg", "-djpeg");
        disp("Graph Export finished")
    end

    if options.SegExport
        segexport(data, segmentsbv, feats, segIDs, clno, "-jpg");
        disp("Segments Export finished")
    end

    if options.StatExport
        statexport(data, stats, statIDs, clno, "-jpg");
        disp("Stat Export finished")
    end

    if options.TypeExport
        typeexport(data, types, typestotal, typeIDs, clno, "-djpeg");
