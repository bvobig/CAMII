...
hold off
cla'
set(handles.pianoroll1,'Units','normalized','Position',[.1,.92,.87,.07]);
...
if length(mchannels(handles.nmat))>1
    ...
    set(handles.pianoroll2,'Units','normalized','Position',[.1,.85,.87,.07]);
...
if nbaxes>0
    ...
    for i=(1:nbaxes)
        axes(listaxes(i));
        cla;
        set(listaxes(i),'Units','normalized','Position',[.1,height*(nbaxes-i)+.01,.87,height-.005]);
        set(listaxes(i),'Visible','on');
        set(listcaptions(i),'Units','normalized','Position',[0,height*(nbaxes-i)+.01,.1,.03]);
...
for i=(nbaxes+1:14)
    ...
    set(listaxes(i),'Units','normalized','Position',[.1,0,.87,.01]);
    set(listaxes(i),'Visible','off');
