...
%      'noylabel' = do not display ylabel
...
nameformat=[]; timeformat=[]; velformat=[];  color=[]; status=[]; ylabeldisplay=1;
...
for i=1:length(varargin)
    ...
    if strcmp(varargin(i),'noylabel'), ylabeldisplay=0; end
	if iscell(varargin(i)) & length(char(varargin(i)))==1
		color=char(varargin(i));
	end
	 switch char(varargin(i))
		  case {'hold','num','name','beat','sec','vel','k','r','g','b','c','m','y','k','w','noylabel'}
		  otherwise
...
if length(m)>1
	for i=1:length(m),
		...
		p=pitch(n);
		proll(n,nameformat,timeformat,velformat,cols(i,:),'hold',ylabeldisplay);
		text(n(1),p(1)+1.5,['ch' num2str(i)],'Color', cols(i,:),'Fontsize',11);
	end
else
		proll(nmat,nameformat,timeformat,velformat,color,status,ylabeldisplay);
end

function proll(nmat,nameformat,timeformat,velformat,color,status,ylabeldisplay);
