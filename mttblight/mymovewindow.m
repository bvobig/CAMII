function y = mymovewindow(nmat,wlength,wstep,tmin,tmax,timetype,varargin)
% Windowed analysis of notematrix using a specified function
% y = movewindow2(nmat,wlength,wstep,tmin,tmax,timetype,varargin)
...
for t = tmin:wstep:tmax
   ...
   % Each frame corresponds to the wlength seconds preceding the current
   % time t.
   
   for f=1:length(varargin)
      if isempty(x) % Return empty for empty matrix
         %if strcmp(varargin{1},'nnotes')
         %    z=[0];
         %else
             if strcmp(varargin{1},'pulseac') ...
                     || strcmp(varargin{1},'myautocor') ...
                     || strcmp(varargin{1},'myautocor_noresonance')
                 z=NaN(1,151);
             else
                if strcmp(varargin{1},'mypulsesalience') || ...
                        strcmp(varargin{1},'interpulsesalience') || ...
                        strcmp(varargin{1},'mypulsesalience_noresonance')
                    z=[NaN NaN];
                else
                    z=[NaN];
                end
